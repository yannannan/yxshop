    ...
    <section className="admin-main">
      ...
      <div className="admin-panel">
        ...
        {loading ? <div className="admin-loading">正在读取数据…</div> : <>
          ...
          {active === 'users' && <><div className="panel-title"><div><h2>用户列表</h2><p>查看注册用户、账号状态与权限类型；10001=普通用户，10002=超级管理员</p></div><div className="panel-title-actions"><button className="refresh-list-button" disabled={refreshing} onClick={() => void load(true)}>{refreshing ? '正在刷新…' : '↻ 刷新列表'}</button></div></div><div className="table-wrap"><table><thead><tr><th>用户</th><th>订单邮箱</th><th>权限类型</th><th>注册时间</th><th>状态</th><th>操作</th></tr></thead><tbody>{data.users.map((user) => { const permissionType = text(user.permission_type) || '10001'; const permissionName = text(user.permission_type_name) || (permissionType === '10002' ? '超级管理员' : '普通用户'); const isCurrentAdmin = text(user.phone) === adminSession.admin.username; return <tr key={text(user.id)}><td><b>{text(user.phone).replace(/(\d{3})\d{4}(\d{4})/, '$1****$2')}</b><small>用户 ID：{text(user.id)}{isCurrentAdmin ? ' · 当前管理员' : ''}</small></td><td>{text(user.email)}</td><td><b>{permissionType}</b><small>{permissionName}</small></td><td>{text(user.created_at).slice(0, 10)}</td><td><Status value={text(user.status)} /></td><td><button className="text-action" disabled={isCurrentAdmin && text(user.status) === 'active'} onClick={() => confirmStatus('变更用户状态', `确定${user.status === 'active' ? '停用' : '启用'}该用户吗？`, { action: 'user-status', id: Number(user.id), status: user.status === 'active' ? 'disabled' : 'active' })}>{user.status === 'active' ? '停用' : '启用'}</button><button className="text-action" disabled={isCurrentAdmin && permissionType === '10002'} onClick={() => confirmStatus('变更用户权限', permissionType === '10002' ? '确定将该用户调整为普通用户吗？调整后将不能访问管理平台。' : '确定将该用户设置为超级管理员吗？设置后可访问管理平台。', { action: 'user-permission', id: Number(user.id), permissionType: permissionType === '10002' ? '10001' : '10002' })}>{permissionType === '10002' ? '设为普通用户' : '设为超级管理员'}</button></td></tr>; })}</tbody></table>{data.users.length === 0 && <Empty text="暂无注册用户" />}</div></>}
