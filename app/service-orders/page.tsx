...
export default function TechnicalServiceOrdersPage() {
  ...
  return <main className="simple-page orders-page">
    <header className="inner-header">
      ...
      <Link href="/orders">商城订单</Link>
    </header>
    <section className="orders-wrap">
      <div className="orders-title"><div><span className="section-kicker">SERVICE ORDER CENTER</span><h1>我的技术服务订单</h1><p>查看报价、预约、接单和技术服务进度</p></div><Link href="/?channel=technical">继续选择技术服务 →</Link></div>
      {loading ? <div className="orders-empty">正在读取技术服务订单…</div> : unauthorized ? <div className="orders-empty"><b>登录后查看技术服务订单</b><p>请使用商城手机号登录后再查看。</p><Link href="/?channel=technical">返回技术服务</Link></div> : !visible.length ? <div className="orders-empty"><b>暂时没有技术服务订单</b><p>选择技术服务并提交预约后，订单会显示在这里。</p><Link href="/?channel=technical">选择技术服务</Link></div> : <div className="orders-list">{visible.map((order) => <article className="order-card" key={order.id}>
        ...
        <div className="order-card-body"><span className="order-product-icon">服</span><div className="order-product"><small>技术服务</small><b>{order.service_title}</b><span>{order.provider_name} · {order.delivery_mode === "onsite" ? "指定地点服务" : "在线服务"}</span></div><div className="order-price"><small>服务金额</small><b>{order.amount === null ? "待报价" : <><em>¥</em>{order.amount}</>}</b></div><div className="order-action"><small>预约：{String(order.scheduled_at).replace("T"," ").slice(0,16)}</small><Link href={`/service-orders/${encodeURIComponent(order.id)}`}>查看订单</Link></div></div>
      </article>)}</div>}
    </section>
  </main>;
}
